           ...
           backgroundColor: Colors.grey[900],
            ...
            appBar: AppBar(                                                          // В объекте указываем шапку, используем AppBar
             ...
             body: ListView
               ...
               itemBuilder: (BuildContext context, int index) {                                                     // itemBuilder это специальное значение, за счет которого мы указываем каким образом будет построен весь список. В качестве значения указываем функцию, эта функция принимает 2 параметра это BuildContext context, второй это индекс с которым мы будем работать int index
                 return Dismissible(
                     ...
                     child: Card(                                                   // В чаилд будем указывать дизайн, за счет которого мы будем отображать каждый отдельный элемент. C помощью Card() создаем некую карточку
                       child: ListTile(
                           title: Text(todoList[index]),                            // Тут указываем что выводить
                          trailing: IconButton(                                     // Добавляем иконку
                            ...
                            onPressed: () {                                         //обрабатываем событие нажатие на кнопку
                                  setState(() {
                                    todoList.removeAt(index);                       // меняем состояние, состояние списка и удаляем определенный элемент по его индексу
                                  });
                                },
                             )
                       ),
                     ),
                 onDismissed: (direction)                                           // обработчик событий к Dismissible для отслеживания событий смахивания. эта функциядолжна принимать параметр direction за счет этого можно понять в какую сторону определенная карточка была удалена
                 {
                   ...
                   setState(() {                                                    // Не важно в какую сторону было смахивание, Мы просто будем удалять из , обращаемся к setState. Через данный метод мы можем контролировать состояние
                     todoList.removeAt(index);                                      // Обращаемся к todoList.removeAt(index туу указаваем индекс который хотим удалить)
               ...
               floatingActionButton: FloatingActionButton(                           // Прописываем кнопку, основывается кнопка на классе FloatingActionButton
                     ...
                     showDialog(context: context, builder: (BuildContext context)) {                  // при нажатии выводим всплывающее окно, вызываем этот метод
                         ...
                         child: Icon(                                                      // дочерний элемент будет обычная иконка
                           Icons.add_box,                                                  // для иконки указываем класс Icons далее выбираем из всех возмодных иконок add и выбираем иконку.
